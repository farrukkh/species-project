...
function* handleGetRegions() {
  const regionsResponse = yield fetch(`http://apiv3.iucnredlist.org/api/v3/region/list?token=${TOKEN}`)
...
function* handleGetSpeciesByRegion(action) {
  ...
  */
  const speciesResponse = yield fetch(`http://apiv3.iucnredlist.org/api/v3/species/region/${region.identifier}/page/0?token=${TOKEN}`)
        .then(response => response.json());
...
function* handleGetSingleSpeciesConservation(specie) {
  ...
  const specieConservationMehods = yield fetch(`http://apiv3.iucnredlist.org/api/v3/measures/species/id/${specie.taxonid}?token=${TOKEN}`)
        .then(response => response.json());

  if (specieConservationMehods.result.length) {
    specieConservationMehods.result.forEach((conservationResult, conservationResultIndex) => {
      specieWithConservationInfo.text = (specieWithConservationInfo.text || '')
...
function* handleGetCRSpecies(action) {
  ...
  const crSpeciesWithConservation = yield all(filteredSpecies.map((specie) => call(handleGetSingleSpeciesConservation, specie)));
...
function* handleGetMammalSpecies(action) {
  const { payload: { species } } = action;
  const filteredSpecies = species.filter((specie) => specie.class_name === 'MAMMALIA');
