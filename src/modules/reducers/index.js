...
const reducer = (state = defaultState, action) => {
  switch (action.type) {
    ...
    case types.GET_SPECIES_BY_REGION:
      return { ...state, isFetching: true, filters: { ...state.filters, crSpecies: false, mammals: false } };
    ...
    case types.SET_SELECTED_FILTER:
      return { ...state, filters: { ...state.filters, [action.payload.filter]: action.payload.selected } };
    case types.SET_CURRENT_REGION:
      return { ...state, filters: { ...state.filters, currentRegion: action.payload.region } };
    default: 
      return state;
  }
};
